<template>
  <div class="w-full flex justify-center pb-5">
    <div class="container max-w-200 mx-auto">
      <Card>
        <template #header>
          <h1 class="p-5 text-lg">Editing regiment data</h1>
        </template>
        <template #content>
...
const legacyMatchLanguages = (languages: Language[]) => {
  return languages.map((language) => {
    const match = languageList.find((lang) => lang.value === language.value)
    return match ? match : language
  })
}

const initialValues = ref({
  name: regimentStore.userRegiment.name,
  slogan: regimentStore.userRegiment.slogan,
  faction: regimentStore.userRegiment.faction.toString(),
  timezones: regimentStore.userRegiment.timezones,
  languages: legacyMatchLanguages(regimentStore.userRegiment.languages),
  shard: regimentStore.userRegiment.shard,
  players: regimentStore.userRegiment.players,
  activities: regimentStore.userRegiment.activities,
  long_desc: regimentStore.userRegiment.long_desc,
  short_desc: regimentStore.userRegiment.short_desc,
  image: regimentStore.userRegiment.image,
  discord: regimentStore.userRegiment.discord,
  established: regimentStore.userRegiment.established.seconds
    ? new Date(regimentStore.userRegiment.established.seconds * 1000)
    : new Date(regimentStore.userRegiment.established),
  clantag: regimentStore.userRegiment.clantag,
  shortlink: regimentStore.userRegiment.shortlink ?? '',
})
const image = ref(regimentStore.userRegiment.image)
...
const onFormSubmit = async (values: any) => {
  ...
  // TODO: fix checking for existing regiments and not getting locked out from updating your own regiment
  ...
  if (values.states.shortlink.value) {
    ...
    if (querySnapshot.docs.length > 0 && querySnapshot.docs[0].id !== regimentStore.userRegiment.id) {
  ...
  // @ts-expect-error Todo: Fix this
  await updateDoc(doc(db, 'regiment', useCurrentUser().value.uid), {
    ...
    .then(() => {
      successMessage.value = 'Regiment listing updated successfully.'
      errorMessage.value = ''
...
const shortDesctext = ref(regimentStore.userRegiment.short_desc)
const longDesctext = ref(regimentStore.userRegiment.long_desc)
